...
export async function handleAdvertisementBidEnd(
  ...
  const bids = await AdvertisementBid.getByAdvertisementId(adId.toString());
  console.log(`got ${bids.length} bids of adId = ${adId}`);
  await Promise.all(
    bids
      .filter((bid) => bid.active === true)
      .filter((bid) => bid.nftId === nftId.toString())
      .filter((bid) => bid.advertisementId === adId.toString())
      .map(async (bid) => {
        console.log(`set bid.active = flase for adId = ${adId}`);
        bid.active = false;
        await bid.save();
      })
  );
